...
function VideoList() {
     ...
     const GetUserVideoList=async()=>{
        const result= await convex.query(api.videoData.GetUserVidoes,{
            uid:user?._id
        });
        setVideoList(result);
        console.log(result);
  ...
  return (
    <div>
        {videoList?.length==0 ?
        <div className='flex flex-col items-center justify-center mt-28 gap-5 p-5 border border-dashed roundex-xl py-16'>
        ...
        <div className = 'grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-5 mt-10'>
            {videoList?.map((video,index)=>(
                ...
                <div className='relative'>
                    {video?.status=='completed'?<Image src={video?.images[0]} alt={video?.title} width={500} height={500} className='w-full object-cover rounded-xl aspect-[2/3]' unoptimized />:
                    ...
                    <div className='absolute bottom-3 px-5 w-full'>
                        <h2>{video?.title}</h2>
                        {/* <h2>{ moment (video?.creationTime).fromNow()}</h2> */}
